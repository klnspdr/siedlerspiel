class Group {
    ...
    draw() {
        if (parseInt(this.groupData['hp']) === 0) {
    ...
    updateShownInfo() {
        if (this.groupData != null && this.inventory != null) {


            let info = this.groupData;
            fill(0);
            strokeWeight(0);
            textSize(15);
            textStyle(NORMAL);
...
class Enemy extends Group {
    ...
    draw() {
        if (this.groupData != null && this.inventory != null) {
            if(this.inventory[config.item_hiding_inventory] >= 1 && !this.statsRevealed ){
