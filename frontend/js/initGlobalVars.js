  ...
  function createPlayers(groupId) {
    ...
    if (groupId !== 100) {
      if (config.groupPositioning === "solid") {
        player = new Player(
          groupId,
          mapSolidX(groupPositions[groupId - 1].x),
          mapSolidY(groupPositions[groupId - 1].y),
          reverseMapX(mapSolidX(groupPositions[groupId - 1].x)),
          reverseMapY(mapSolidY(groupPositions[groupId - 1].y)),
          { ...groupIconSize },
        );
        for (let i = 1; i <= config["number_groups"]; i++) {
          if (i !== groupId) {
            enemies.push(
              new Enemy(
                i,
                mapSolidX(groupPositions[i - 1].x),
                mapSolidY(groupPositions[i - 1].y),
                reverseMapX(mapSolidX(groupPositions[i - 1].x)),
                reverseMapY(mapSolidY(groupPositions[i - 1].y)),
                { ...groupIconSize },
              ),
            );
          } else {
        ...
        for (let i = 1; i <= config["number_groups"]; i++) {
          if (i !== groupId) {
            let angle =
              (TWO_PI / config["number_groups"]) * i +
              PI / 2 -
              (PI / config["number_groups"]) * 2 * player.groupId;
...
function initGlobalVars() {
  $.getJSON("/ajax/getRole").done(function (data) {
    groupId = parseInt(data.role, 10) || 0;
    document.title =
      groupId === 0
        ? "Siedler - Konfiguration"
        : "Siedler - " +
          (groupId !== 100 ? config.group_names[`gr${groupId}`] : "Überblick");
  });
